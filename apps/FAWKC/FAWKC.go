...
func (a *App) GetFAWKC(Authorization string, batchId string) (map[string]interface{}, error) {
	// 函数入口处直接校验并上报
	if err := only9464.ReportJWTusername(Authorization); err != nil {
		return nil, err
	}
	// 构造请求数据
	...
	for _, c := range resp.Cookies() {
		if c.Name == "JSESSIONID" {
			fmt.Println("[GetFAWKC]更新之前的全局JSESSIONID:", only9464.GOLBALJSESSIONID)
			fmt.Println("[GetFAWKC]从响应中获取到JSESSIONID:", c.Value)
			only9464.GOLBALJSESSIONID = c.Value
			fmt.Println("[GetFAWKC]更新后的全局JSESSIONID:", only9464.GOLBALJSESSIONID)
			break
		}
	}
	// 解析响应
